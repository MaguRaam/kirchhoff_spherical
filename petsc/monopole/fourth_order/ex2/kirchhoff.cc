...
//! THere is some issue: Not getting fourth-order convergence yet
...
int main(){
    ...
    // find minimum and maximum emission time:
    auto [min, max] = std::minmax_element(tauq.origin(), tauq.origin() + tauq.num_elements());

    std::cout << "min emission time = " << *min << std::endl;
    std::cout << "max emission time = " << *max << std::endl;
