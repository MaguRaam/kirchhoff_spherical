...
inline auto interpolate_pressure(const std::vector<double> &t, double tau, int theta_index, const boost::multi_array<KirchhoffData, 2> &data)
{
    ...
    double p_tau = lagrange_interpolation<4>({t[i - 1], t[i], t[i + 1], t[i + 2]}, {data[i - 1][theta_index].p, data[i][theta_index].p, data[i + 1][theta_index].p, data[i + 2][theta_index].p}, tau);
    double pr_tau = lagrange_interpolation<4>({t[i - 1], t[i], t[i + 1], t[i + 2]}, {data[i - 1][theta_index].pr, data[i][theta_index].pr, data[i + 1][theta_index].pr, data[i + 2][theta_index].pr}, tau);
    double pt_tau = lagrange_derivative<4>({t[i - 1], t[i], t[i + 1], t[i + 2]}, {data[i - 1][theta_index].p, data[i][theta_index].p, data[i + 1][theta_index].p, data[i + 2][theta_index].p}, tau);
