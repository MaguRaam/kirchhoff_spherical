...
//----------------------------------------------------------------------------
PetscReal WriteKirchhoffData(Vec U, DM da, AppCtx* Ctx, PetscReal time){

    PetscErrorCode  ierr;
    PetscInt q, i, j, k, nproc, rank;
    PetscReal coeffs[nDOF], pcoeffs[nDOF];
    PetscReal xc, yc, psi, eta, px, py, nx, ny, h = Ctx->h, po;
    Field **u;
    ...
    //compute pressure at observer point:
    i = GetCellIndex(Ctx->xo, Ctx->x_min, Ctx->h);
    j = GetCellIndex(Ctx->yo, Ctx->y_min, Ctx->h);
    ...
    //compute pressure at quadrature point:
    po = evaluate_polynomial(psi, eta, coeffs);
