...
PetscErrorCode MonitorFunction (TS ts,PetscInt step, PetscReal time, Vec U, void *ctx) {
    ...
    FILE *p_file, *t_file, *po_file;    


    if (step%Ctx->KirchhoffWriteInterval == 0){

        //compute p', p'r on Kirchhoff surface:			
        PetscReal po =  WriteKirchhoffData(U, da, Ctx, time); CHKERRQ(ierr);


        if (MyPID == 0) {
            t_file = fopen("kirchhoff/data/time.dat","a+");
            p_file = fopen("kirchhoff/data/pressure.dat","a+");
            po_file = fopen("kirchhoff/p_weno.dat","a+");

            //write time.dat
            ierr = PetscFPrintf(PETSC_COMM_SELF, t_file, "%.16e\n", time); CHKERRQ(ierr);
            
            //loop over quadrature points and write pressure data:
            ierr = PetscFPrintf(PETSC_COMM_SELF, p_file, "%.16e\n", time); CHKERRQ(ierr);
            ...
            //write perturbed pressure at observer point:
            ierr = PetscFPrintf(PETSC_COMM_SELF, po_file, "%.16e\t%.16e\n", time, po); CHKERRQ(ierr);
