...
void RayleighCollapse(PetscReal x, PetscReal y, PetscReal *Q0)
{
    ...
    PetscReal p_inf = 1.0;  // Pressure in the water mediumn
    PetscReal p_b =  0.1;   // Pressure inside the air bubble

    PetscReal rho_inf = 1.0;   // Far field density
    PetscReal rho_b = 0.001;   // Bubble density

    V0[0] = TanhRadial(x, y, x0, y0, R0, smear * h, rho_b, rho_inf);
    V0[1] = 0.0;
    V0[2] = 0.0;
    V0[3] = TanhRadial(x, y, x0, y0, R0, smear * h, p_b, p_inf);
    V0[4] = TanhRadial(x, y, x0, y0, R0, smear * h, 0.0, 1.0);
