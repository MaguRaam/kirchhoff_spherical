...
int main()
{
    ...
    // wave speed:
    const double c = 1624.9448698340507;

    // bubble radius:
	const double R0 = 6.0e-5;

    // radius of sphere:
    const double R = 5*R0;

    // observer point P1 from Jamalludin paper:
    const double theta_o = 3*M_PI;
    const double phi_o = M_PI/3.0;
    const double rb = 0.15;
    
    const Point xo{rb*sin(theta_o)*sin(phi_o), rb*cos(phi_o), rb*cos(theta_o)*sin(phi_o)};

    // observer time:
    double to = 0.00;
    const double tf = 1.e-6;
