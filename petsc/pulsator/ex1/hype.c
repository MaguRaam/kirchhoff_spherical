static char help[] = "Write pressure data from sound waves emitted by rigid pulsator to second order Kirchhoff solver.\n\n";
...
int main(int argc,char **argv){
    ...
    Ctx.N_x             =  200;
    Ctx.N_y             =  100;
    Ctx.dt              =  1.313e-6/100.0;  // time period / no of time steps
    Ctx.InitialStep     =  0;
    Ctx.InitialTime     =  0.0;
    Ctx.FinalTime       =  335.e-6;
    ...
    while (it*Ctx.dt < Ctx.FinalTime){
        ...
        ierr = ComputePressureAverage(U, da, it*Ctx.dt, Ctx); CHKERRQ(ierr);
