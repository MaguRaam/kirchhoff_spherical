...
int main()
{
    ...
    // radius of sphere:
    const double R = 70.001;

    // observer point:
    const Point xo{90.001, 0.0, 0.0};

    // observer time:
    double to = 7.0417941736349521e+00;
    ...
    // data filename:
    std::string filename = "data/R=70.001000.dat";
    ...
    auto [rq, cosq] = compute_distance_angle(xq, xo);
		...
		std::cout << "compute emission time\n";
		auto tauq = compute_emission_time(xq, xo, to, c);
		
		// compute Kirchhoff integral:
    double error = fabs(compute_kirchhoff_integral(rq, cosq, tauq, t, p, pr, R, c, theta) - 	1.1123158771169983e-02);
